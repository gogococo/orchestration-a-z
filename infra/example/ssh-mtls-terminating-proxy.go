...
func init() {
	if len(os.Args) <= 1 {
		...
		var (
			...
			consulCLICertFile   string
			consulCLIKeyFile    string
		)
		...
		flag.StringVar(&consulCACertFile, "consul-ca-file", "", "mTLS certifcate authority file")
		flag.StringVar(&consulCLICertFile, "consul-cert-file", "", "mTLS client cert file")
		flag.StringVar(&consulCLIKeyFile, "consul-key-file", "", "mTLS client key file")
		flag.Parse()
		...
		consulCACert = readFileContent(consulCACertFile)
		consulCLICert = readFileContent(consulCLICertFile)
		consulCLIKey = readFileContent(consulCLIKeyFile)
	}
